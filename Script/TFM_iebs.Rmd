...
Disponer de una herramienta que permita analizar las componentes frecuenciales de un grupo de muestras de sonido, permitiendo la visualización en un espacio bidimensional de sus diferencias para una rápida identificación. 
...
Ut mollis non enim ultricies fermentum. In ac aliquet dui, eget interdum magna. Suspendisse gravida volutpat nisl, non consequat neque porttitor eu. Aliquam fermentum porttitor turpis at facilisis. Phasellus iaculis purus et risus bibendum, eget tincidunt mauris imperdiet. Integer id ultricies neque. Proin varius nisl eu convallis placerat. Mauris ultricies mattis hendrerit. Nulla vitae urna nec lectus porta vestibulum. Donec gravida, dolor eget maximus blandit, neque tortor tristique justo, nec dapibus nisi nisi ac urna. Morbi ultricies arcu maximus est gravida, sit amet rhoncus turpis ornare.
...
Sononym. "navegador de muestras de audio", utilidad que permite la catalogación de las muestras de audio a traves del analisis de la muestra.
...
Visualizar la relación frecucial existente hay entre los elementos de un conjunto de muestras de sonido.
...
2. Comparar el contenido frecuencial de referencia mediante la correlación cruzada tiempo-frecuencia y las demás muestras.
...
La metodología utilizada se basará en el uso de unas muestras de audio a modo de referencia que permitirán comprobar el funcionamiento correcto del modelo.

Tres conjuntos de muestras a modo de referencia con una serie de caracteristas particulares ayudan a determinar el correcto funcionamiento del modelo:
...
\  Conjunto C: Muestras sin ningún tipo de relación timbrica entre ellas y de misma duración que las anteriores (outlayers del conjunto) pretenden ser los elementos disruptivos en el analisis del conjunto.

\vspace{15pt}

1. Instrumentos

Los instrumentos utilizados en la investigación han sido:
...
<center>
[![fig.1](/home/ion/TFM/0Coast.jpeg)](fig.1) 
</center>

El motivo por el que se elige la utilización de este sintetizador, es que permite la utilización de técnicas de síntesis empleadas en sistemas modulares que incluyenn la adición de sobretonos armónicos en la señal que hayamos generado.
...
Para llevar a cabo el proyecto se relacionó las muestras de sonido con el ejemplo de escalamiento multidimensional, para ello se hizo uso de warbleR, biblioteca de funciones de que tiene por objeto facilitar el análisis de la estructura de las señales acústicas de los animales en R. 
...
hacer uso de warbleR y de las librerias de las que depende, que tiene por objeto facilitar el análisis de la estructura de las señales acústicas de los animales en R, herramienta ideal para el analisis de  las muestras que necesitabamos hacerLos usuarios pueden recopilar grabaciones aviares de libre acceso o introducir sus propios datos en un flujo de trabajo que facilite la visualización espectrográfica y la medición de los parámetros acústicos. warbleR hace uso de las herramientas de análisis de sonido fundamentales del paquete de ondas marinas y ofrece nuevas herramientas para el análisis de la estructura acústica. Estas herramientas están disponibles para el análisis por lotes de señales acústicas.


Una vez encontrada la manera de poder analizar las muestras de sonido , organizaron un seminario informativo con la dirección y el profesorado para explicarles los objetivos de la investigación y para obtener las autorizaciones de los padres (ya que era un estudio en el que participan menores de edad).
...
```{r 02 limites establecidos en el filtro pasabanda}
...
```{r 03 Representación espectral de la muestra de referencia }
# PLOT
