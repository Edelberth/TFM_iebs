...
Aproximación a una de las técnicas de clasificación que forman parte de la rama del Machine Learning y el aprendizaje no supervisado, referente a la reducción de dimensionalidad y concretamente al metodo de escalamiento multidimensional.   


Para ello se comparará una de las muestras de audio, la que en adelante será tomada como referencia respecto al grupo y mediante el analisis de sus frecuencias entre todos los elementos del grupo (este será nuestro dataset) nos dará un resultado que será representado en un eje de coordenadas donde se podrá observar como de lejanas o cercanas son entre ellas.  
...
La metodología utilizada se basará en unas muestras de audio a modo de referencia que permitirán comprobar el funcionamiento correcto del modelo. Tres conjuntos de muestras a modo de referencia con una serie de caracteristas particulares ayudan a determinar el correcto funcionamiento del modelo:
...
Creamos un data frame que posteriormente necesitaremos, en el crearemos las columnas con:

*nombre de la muestra que vamos a tomar como referencia. 
*numero de canal 
*tiempo de inicio de muestra, 
*tiempo de final de muestra  
*"selec" = 2.  
...
Creamos un objeto "**wave**" a partir del único nombre que tenemos el dataframe "**plantilla**"

El objeto "**wave**" tendrá una duración de x segundos tal, de hecho ya lo habiamos indicado en el dataframe y este será el tiempo con el que compararemos todas las demás muestras.
...
Establecemos la frecuencia (KHz) que queremos visualizar en el eje Y del grafico 

Para ello vamos a determinar:

Cual es la frecuencia mas alta que contiene nuestra muestra

\vspace{15pt}


```{r frecuencia clip}
pico_freq_max <- fpeaks(spec(wave_referencia,dB = "max0",plot=TRUE ),nmax = 1)
```
...
high_f <- (pico_freq_max[c(1,0)]) * zoom_X_10 # matrix to numeric
low_f = 0
```
...
fundamental_wave_referencia <- autoc(wave_referencia, threshold=0.01, plot=FALSE) # fundamental frequency
...
points(fundamental_wave_referencia, pch=25)
...
\ **ovlp**  = permite la superposición entre dos ventanas sucesivas .
...
\ **flim**           = modificaciones en los límites del eje Y de la frecuencia (en kHz).
...
\  **grid**      = visualización de la rejilla.	
...
Combinamos el dataframe "**plantilla**" con el dataframe anterior "**sel_tab_1**" en un nuevo dataframe llamado "**sel_tab**"

En esta combinación entre dataframes observamos que tenemos repetido el nombre elegido como referencia, con la particularidad de que la columna selec es diferente en uno y en el otro caso.
...
Como en el inicio creamos el dataframe "**plantilla**" con una columna selec = 2 ahora esta columna nos permite diferenciar entre nombres iguales. 
...
En el procesamiento de señales, la correlación cruzada es una medida de similitud de dos series en función del desplazamiento en este caso de frecuencias de una con respecto a la otra. Tiene aplicaciones en reconocimiento de patrones, análisis de partículas individuales, tomografía electrónica, promediado, criptoanálisis y neurofisiología. 
...
## Visualizacion en Boxplot y diagrama de dispersion 

\vspace{15pt}

```{r Boxplot entre las muestras, plot=TRUE }
...
ref_X_pitch_2 <- autoc(wave_referencia, threshold=thrhld, plot=FALSE)
ref_X_pitch_mono <- mono(wave_referencia)
ref_X_pitch_f_max <- fpeaks(spec(ref_X_pitch_mono),nmax = 1)

X_Hit <- read_wave(sel_tab_1$sound.files[1])
ref_X_Hit <- autoc(X_Hit, threshold=thrhld, plot=FALSE)
X_Hit_mono <- mono(X_Hit)
X_Hit_f_max <- fpeaks(spec(X_Hit_mono),nmax = 1)

X_Low <- read_wave(sel_tab_1$sound.files[2])
ref_X_Low <- autoc(X_Low, threshold=thrhld, plot=FALSE)
X_Low_mono <- mono(X_Low)
X_Low_f_max <- fpeaks(spec(X_Low_mono),nmax = 1)
...
hf <- as.numeric(ref_X_pitch_f_max[1,1])

Ws_X_pitch <- periodogram(ref_X_pitch_mono, 
                        frqRange = c(0, hf*10000),width = 2^13 )
plot(Ws_X_pitch)
title(main = list("X_pitch (ref)", cex = 1.5,col = "red", font = 3))



hf <- as.numeric(X_Hit_f_max[1,1])

Ws_X_Hit <- periodogram(X_Hit_mono, 
                        frqRange = c(0, hf*10000),width = 2^13 )
plot(Ws_X_Hit)
title(main = list("X_Hit ", cex = 1.5,col = "red", font = 3))




hf <- as.numeric(X_Low_f_max[1,1])
Ws_X_Low <- periodogram(X_Low_mono, 
                        frqRange = c(0, hf*10000),width = 2^13 )
plot(Ws_X_Low)
title(main = list("X_Low ", cex = 1.5,col = "red", font = 3))
...
X_pitch_6 <- read_wave(sel_tab_1$sound.files[4])
ref_X_pitch_6 <- autoc(X_pitch_6, threshold=thrhld, plot=FALSE)
plot(ref_X_pitch_6, type = tipo, xlab = "Componente frecuencial", ylab = "Amplitud")
title(main = list("X_pitch_6 ", cex = 1.5,col = "red", font = 3))


X_pitch_17 <- read_wave(sel_tab_1$sound.files[5])
ref_X_pitch_17 <- autoc(X_pitch_17, threshold=thrhld, plot=FALSE)
plot(ref_X_pitch_17, type = tipo, xlab = "Componente frecuencial", ylab = "Amplitud")
title(main = list("X_pitch_17 ", cex = 1.5,col = "red", font = 3))
...
plot(xcor, type = "b", xlab = "Simililltud Frecuencial", ylab = "Tímbrica")
...
plot(graf, type ="b", layout = layout, vertex.size = 25)
```
